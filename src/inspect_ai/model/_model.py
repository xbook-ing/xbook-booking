...
from xbook._util.constants import DEFAULT_MAX_CONNECTIONS
from xbook._util.content import ContentText
from xbook._util.hooks import init_hooks, override_api_key, send_telemetry
...
from xbook._util.retry import log_rate_limit_retry
from xbook.tool import Tool, ToolChoice, ToolFunction, ToolInfo
from xbook.tool._tool_def import ToolDef, tool_defs
from xbook.util import concurrency
...
class Model:
    ...
    def verify_model_apis(self) -> None:
        if (
            os.getenv("xbook_DISABLE_MODEL_API", None) is not None
            and ModelName(self).api != "mockllm"
        ):
            raise RuntimeError("Model APIs disabled by xbook_DISABLE_MODEL_API")
    ...
    def _record_model_interaction(
        ...
        from xbook.log._transcript import ModelEvent, transcript
    ...
    **model_args: Any,
    ...
    Args:
       model (str | Model | None): Model specification.
         ...
         then the model referred to by `xbook_EVAL_MODEL`).
    ...
    if model is None:
        ...
        model = os.getenv("xbook_EVAL_MODEL", None)
        if model is not None:
            model = model.split(",")[0]
        else:
            raise ValueError("No model specified (and no xbook_EVAL_MODEL defined)")
...
def resolve_models(
    ...
    if model is None or isinstance(model, str):
        model = model or os.getenv("xbook_EVAL_MODEL", None)
        if model is None:
            raise ValueError("No model specified (and no xbook_EVAL_MODEL defined)")
...
def record_model_usage(model: str, usage: ModelUsage) -> None:
    ...
    from xbook.log._samples import set_active_sample_total_tokens
