...
from xbook._util.constants import SANDBOX
...
def task_project_name(task: str) -> str:
    ...
    return f"xbook-{task}-i{uuid().lower()}"


xbookproject_pattern = r"^xbook-[a-z\d\-_]*-i[a-z\d]{22}$"


def is_xbookproject(name: str) -> bool:
    return re.match(xbookproject_pattern, name) is not None
