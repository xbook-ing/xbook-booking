...
from xbook.util._subprocess import ExecResult, subprocess

from .prereqs import (
    DOCKER_COMPOSE_REQUIRED_VERSION_PULL_POLICY,
    validate_docker_compose,
)
from .util import ComposeProject, is_xbookproject, sandbox_log
...
async def compose_ls() -> list[Project]:
    ...
    if result.success:
        ...
        projects = list(filter(lambda p: is_xbookproject(p["Name"]), projects))
