...
from xbook._util.logger import init_logger
from xbook._view.server import view_server
...
def view(
    ...
    log_dir = log_dir if log_dir else os.getenv("xbook_LOG_DIR", "./logs")
...
def view_acquire_port(port: int) -> None:
    ...
    if pid_file.exists():
        ...
        try:
            ...
            p.terminate()
            display().print(
                f"Terminating existing xbook view command using port {port}"
            )
            p.wait(WAIT_SECONDS)
