...
from xbook._cli.util import parse_cli_args
from xbook._display.core.active import display
from xbook._util.config import resolve_args
from xbook._util.constants import DEFAULT_LOG_FORMAT
from xbook._util.error import PrerequisiteError
from xbook._util.file import absolute_file_path
from xbook._util.platform import platform_init
from xbook._util.registry import registry_lookup
from xbook.approval._apply import init_tool_approval
from xbook.approval._policy import (
    ApprovalPolicy,
    ApprovalPolicyConfig,
    approval_policies_from_config,
    config_from_approval_policies,
)
from xbook.log import EvalConfig, EvalLog, EvalLogInfo
from xbook.log._file import read_eval_log_async
from xbook.log._recorders import create_recorder_for_format
from xbook.model import (
    GenerateConfig,
    GenerateConfigArgs,
    Model,
)
from xbook.model._model import init_active_model, resolve_models
from xbook.scorer._reducer import reducer_log_names
from xbook.solver._chain import chain
from xbook.solver._solver import Solver, SolverSpec
from xbook.util import SandboxEnvironmentType
from xbook.util._trace import init_trace
    ...
    **kwargs: Unpack[GenerateConfigArgs],
    ...
    Args:
        ...
        model (str | Model | list[str] | list[Model] | None): Model(s) for
            evaluation. If not specified use the value of the xbook_EVAL_MODEL
    ...
    **kwargs: Unpack[GenerateConfigArgs],
    ...
    try:
        ...
        if len(resolved_tasks) == 0:
            log.warning("No xbook tasks were found at the specified paths.")
            return []
        ...
        # resolve recorder
        log_dir = log_dir if log_dir else os.environ.get("xbook_LOG_DIR", "./logs")
...
def eval_init(
    ...
    **kwargs: Unpack[GenerateConfigArgs],
    ...
    if len(model_args) == 0:
        env_model_args = os.environ.get("xbook_EVAL_MODEL_ARGS", None)
        if env_model_args:
