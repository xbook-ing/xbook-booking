...
from xbook.tool._tool import Tool, ToolError, tool
from xbook.tool._tool_call import ToolCall, ToolCallContent, ToolCallView
from xbook.tool._tool_info import parse_tool_info
from xbook.tool._tool_with import tool_with
from xbook.util._sandbox import SandboxEnvironment, sandbox_with
from xbook.util._sandbox.docker.internal import xbook_WEB_BROWSER_IMAGE_DOCKERHUB
...
async def web_browser_sandbox() -> SandboxEnvironment:
    ...
    if sb:
        ...
        msg = dedent(f"""
                The web browser service was not found in any of the sandboxes for this sample. Please add the web browser service to your configuration. For example, the following Docker compose file uses the {xbook_WEB_BROWSER_IMAGE_DOCKERHUB} image as its default sandbox:

                services:
                  default:
                    image: "{xbook_WEB_BROWSER_IMAGE_DOCKERHUB}"
                    init: true
