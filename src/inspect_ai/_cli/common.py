...
from xbook._util.display import init_display_type
...
def log_level_options(func: Callable[..., Any]) -> Callable[..., click.Context]:
    ...
    @click.option(
        ...
        envvar="xbook_LOG_LEVEL_TRANSCRIPT",
...
def common_options(func: Callable[..., Any]) -> Callable[..., click.Context]:
    ...
    @click.option(
        ...
        envvar="xbook_NO_ANSI",
    )
    @click.option(
        "--debug", is_flag=True, envvar="xbook_DEBUG", help="Wait to attach debugger"
    )
    @click.option(
        "--debug-port",
        default=5678,
        envvar="xbook_DEBUG_PORT",
