...
from xbook._cli.common import CommonOptions, common_options, process_common_options
from xbook._util.constants import PKG_PATH
from xbook.log import list_eval_logs
from xbook.log._convert import convert_eval_logs
from xbook.log._file import (
    eval_log_json_str,
    read_eval_log,
...
def log_command() -> None:
    ...
    xbook supports two log formats: 'eval' which is a compact, high performance binary format and 'json' which represents logs as JSON.

    The default format is 'eval'. You can change this by setting the xbook_LOG_FORMAT environment variable or using the --log-format command line option.

    The 'log' commands enable you to read xbook logs uniformly as JSON no matter their physical storage format, and also enable you to read only the headers (everything but the samples) from log files, which is useful for very large logs.
    """
    return None
