...
from xbook import Epochs, eval, eval_retry
from xbook._eval.evalset import eval_set
from xbook._util.constants import (
    DEFAULT_EPOCHS,
    DEFAULT_MAX_CONNECTIONS,
    DEFAULT_MAX_RETRIES,
)
from xbook._util.file import filesystem
from xbook._util.samples import parse_sample_id, parse_samples_limit
from xbook.log._file import log_file_info
from xbook.model import GenerateConfigArgs
from xbook.scorer._reducer import create_reducers
from xbook.solver._solver import SolverSpec
...
NO_SCORE_HELP = (
    "Do not score model output (use the xbook score command to score output later)"
)
...
def eval_options(func: Callable[..., Any]) -> Callable[..., click.Context]:
    ...
    @click.option(
        ...
        multiple=True,
        type=str,
        envvar="xbook_EVAL_MODEL_ARGS",
        help="One or more native model arguments (e.g. -M arg=value)",
    )
    @click.option(
        "--model-config",
        type=str,
        envvar="xbook_EVAL_MODEL_CONFIG",
    ...
    @click.option(
        ...
        multiple=True,
        type=str,
        envvar="xbook_EVAL_SOLVER_ARGS",
        help="One or more solver arguments (e.g. -S arg=value)",
    )
    @click.option(
        "--solver-config",
        type=str,
        envvar="xbook_EVAL_SOLVER_CONFIG",
    ...
    @click.option(
        ...
        envvar="xbook_EVAL_TAGS",
    )
    @click.option(
        "--trace",
        type=bool,
        is_flag=True,
        envvar="xbook_EVAL_TRACE",
        help="Trace message interactions with evaluated model to terminal.",
    )
    @click.option(
        "--approval",
        type=str,
        envvar="xbook_EVAL_APPROVAL",
    ...
    @click.option(
        ...
        envvar="xbook_EVAL_EPOCHS_REDUCER",
    )
    @click.option(
        "--max-connections",
        type=int,
        help=MAX_CONNECTIONS_HELP,
        envvar="xbook_EVAL_MAX_CONNECTIONS",
    )
    @click.option(
        "--max-retries",
        type=int,
        help=MAX_RETRIES_HELP,
        envvar="xbook_EVAL_MAX_RETRIES",
    )
    @click.option(
        "--timeout", type=int, help=TIMEOUT_HELP, envvar="xbook_EVAL_TIMEOUT"
    )
    @click.option(
        "--max-samples",
        type=int,
        help=MAX_SAMPLES_HELP,
        envvar="xbook_EVAL_MAX_SAMPLES",
    )
    @click.option(
        "--max-tasks", type=int, help=MAX_TASKS_HELP, envvar="xbook_EVAL_MAX_TASKS"
    )
    @click.option(
        "--max-subprocesses",
        type=int,
        help=MAX_SUBPROCESSES_HELP,
        envvar="xbook_EVAL_MAX_SUBPROCESSES",
    )
    @click.option(
        "--message-limit",
        type=int,
        help="Limit on total messages used for each sample.",
        envvar="xbook_EVAL_MESSAGE_LIMIT",
    )
    @click.option(
        "--token-limit",
        type=int,
        help="Limit on total tokens used for each sample.",
        envvar="xbook_EVAL_TOKEN_LIMIT",
    )
    @click.option(
        "--time-limit",
        type=int,
        help="Limit on total execution time for each sample.",
        envvar="xbook_EVAL_TIME_LIMIT",
    )
    @click.option(
        "--fail-on-error",
        type=float,
        ...
        envvar="xbook_EVAL_FAIL_ON_ERROR",
    )
    @click.option(
        "--no-fail-on-error",
        type=bool,
        ...
        envvar="xbook_EVAL_NO_FAIL_ON_ERROR",
    )
    @click.option(
        "--no-log-samples",
        type=bool,
        ...
        envvar="xbook_EVAL_NO_LOG_SAMPLES",
    )
    @click.option(
    ...
    @click.option(
        "--log-buffer", type=int, help=LOG_BUFFER_HELP, envvar="xbook_EVAL_LOG_BUFFER"
    )
    @click.option(
        "--no-score",
        type=bool,
    ...
    @click.option(
        ...
        envvar="xbook_EVAL_FREQUENCY_PENALTY",
    )
    @click.option(
        "--presence-penalty",
        type=float,
    ...
    @click.option(
        ...
        envvar="xbook_EVAL_TOP_LOGPROBS",
    )
    @click.option(
        ...
        envvar="xbook_EVAL_PARALLEL_TOOL_CALLS",
    )
    @click.option(
        "--max-tool-output",
        type=int,
        help="Maximum size of tool output (in bytes). Defaults to 16 * 1024.",
        envvar="xbook_EVAL_MAX_TOOL_OUTPUT",
    )
    @click.option(
        "--cache-prompt",
        ...
        envvar="xbook_EVAL_CACHE_PROMPT",
    )
    @click.option(
        "--log-format",
        type=click.Choice(["eval", "json"], case_sensitive=False),
        envvar=["xbook_LOG_FORMAT", "xbook_EVAL_LOG_FORMAT"],
...
@click.option(
    ...
    envvar="xbook_EVAL_RETRY_ATTEMPS",
)
@click.option(
    "--retry-wait",
    type=int,
...
@click.option(
    ...
    envvar="xbook_EVAL_NO_RETRY_CLEANUP",
)
@click.option(
    "--bundle-dir",
    type=str,
...
@click.option(
    "--max-samples", type=int, help=MAX_SAMPLES_HELP, envvar="xbook_EVAL_MAX_SAMPLES"
)
@click.option(
    "--max-tasks", type=int, help=MAX_TASKS_HELP, envvar="xbook_EVAL_MAX_TASKS"
)
...
@click.option(
    "--max-retries", type=int, help=MAX_RETRIES_HELP, envvar="xbook_EVAL_MAX_RETRIES"
)
@click.option("--timeout", type=int, help=TIMEOUT_HELP, envvar="xbook_EVAL_TIMEOUT")
@common_options
