...
class LogHandler(RichHandler):
    def __init__(self, levelno: int, transcript_levelno: int) -> None:
        ...
        file_logger = os.environ.get("xbook_PY_LOGGER_FILE", None)
        ...
        file_logger_level = os.environ.get("xbook_PY_LOGGER_LEVEL", "")
...
def init_logger(
    ...
    log_level = (
        log_level if log_level else os.getenv("xbook_LOG_LEVEL", DEFAULT_LOG_LEVEL)
    ).upper()
    ...
    log_level_transcript = (
        ...
        else os.getenv("xbook_LOG_LEVEL_TRANSCRIPT", DEFAULT_LOG_LEVEL_TRANSCRIPT)
    ).upper()
...
def notify_logger_record(record: LogRecord, write: bool) -> None:
    ...
    from xbook.log._transcript import LoggerEvent, transcript
