...
def ensure_entry_points(package: str | None = None) -> None:
    ...
    global _xbookai_eps_loaded_all
    if _xbookai_eps_loaded_all:
        return None

    # have we already loaded entry points for this package?
    if package in _xbookai_eps_loaded:
        return None

    # enumerate entry points
    eps = entry_points(group="xbook")
    for ep in eps:
        try:
...
# xbook extension entry points
_xbookai_eps_loaded: list[str] = []
_xbookai_eps_loaded_all: bool = False
