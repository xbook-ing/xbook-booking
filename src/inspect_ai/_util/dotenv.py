...
xbook_LOG_DIR_VAR = "xbook_LOG_DIR"
...
def init_dotenv() -> None:
    ...
    if dotenv_file:
        # is there an xbook_LOG_DIR currently in the environment? (we will give it preference)
        environment_log_dir = os.environ.get(xbook_LOG_DIR_VAR, None)
        dotenv_log_dir = dotenv_values(dotenv_file).get(xbook_LOG_DIR_VAR, None)
        ...
        if environment_log_dir == dotenv_log_dir or not environment_log_dir:
            ...
            xbooklog_dir = absolute_file_path(environment_log_dir)
        ...
        if xbooklog_dir:
            os.environ[xbook_LOG_DIR_VAR] = xbooklog_dir
