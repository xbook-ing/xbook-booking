...
def _import_object(
    ...
    if msg is None:
        if xbook.isclass(_object):
            _type = "class"
        elif xbook.isfunction(_object):
            _type = "function"
        else:
            _type = "attribute"
        msg = (
...
def deprecation_warning(
    ...
    if calling_frame is not None:
        info = xbook.getframeinfo(calling_frame)
...
def default_deprecation_msg(
    ...
    """
    if user_msg is None:
        if xbook.isclass(obj):
            _obj = " class"
        elif xbook.ismethod(obj):
            _obj = " method"
        elif xbook.isfunction(obj) or xbook.isbuiltin(obj):
            _obj = " function"
        else:
