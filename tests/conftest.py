...
def pytest_sessionfinish(session, exitstatus):
    if importlib.util.find_spec("xbookpackage"):
        try:
            subprocess.check_call(
                [sys.executable, "-m", "pip", "uninstall", "-y", "xbookpackage"]
            )
            shutil.rmtree("tests/test_package/build")
            shutil.rmtree("tests/test_package/xbookpackage.egg-info")
        except subprocess.CalledProcessError as ex:
            warnings.warn(f"Error occurred uninstalling xbookpackage: {ex}")
