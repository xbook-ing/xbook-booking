...
- Add support for links which open the log viewer directly in VSCode. You may copy links to remote log files from the logs panel of the xbook Activity Panel.
...
- When running and debugging xbook evals, the extension will by default use any environments present in the task folder or parent folders (up to the workspace). To always use the workspace environment, change the 'Use Subdirectory Environments` setting.
...
- Improved behavior when previewing json files using the xbook viewer.

## 0.3.37

This version includes a signficant rework of the overall workflow for interacting with xbook. Changes include:

- The xbook sidebar now includes a section which allows you to browse and open log files in the viewer. 
- The xbook Viewer is no longer opened automatically when an evaluation is completed. Instead a notification is available to open the viewer. 
- New open log directory command allows you to select a log directory and open the xbook Viewer for that directory
- Support for the new xbook `eval` log format.

## 0.3.36

- Show xbook version in status bar
...
- Don't restrict the availability of the `Open with xbook View` command based upon the filename.

## 0.3.30

- Further refinement to xbook View behavior when a task is complete. We will now refresh the viewer without bringing it the foreground when it is already displaying and a task completes. This will ensure that focus is handled correctly in the editor. Use `cmd+shift+L` to bring the viewer to the foreground (or show it if it isn't showing).
...
- Minor fix to view rendering across differing versions of xbook.
...
- Right click on log files in the Explorer sidebar and select `Open with xbook View` to view a log file
...
- Fix an issue showing the log viewer when an evaluation completes (specific to xbook 0.3.10 or later)

## 0.3.18

- Fix issues with task params when type hints are provided
- Improve metric appearance in `xbook view`

## 0.3.17

- Improve `xbook view` title bar treatment
...
- Improve xbook binary detection on Linux platforms

## 0.3.13

-   Ensure that xbook CLI is in the path for terminals using a global Python environment
...
-   Fix 'Restart' when debugging to properly restart the xbook debugging session
