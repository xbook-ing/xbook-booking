...
export function activateProtocolHandler(context: ExtensionContext) {
  const protocolHandler = new xbookProtocolHandler();
  context.subscriptions.push(window.registerUriHandler(protocolHandler));
}

export class xbookProtocolHandler implements UriHandler {
    ...
    switch (command) {
      ...
      case "open": {
        ...
        if (logFile) {
          ...
          await commands.executeCommand('xbook.openLogViewer', logUri);
        }
      }
    }
  }
