...
export class WorkspaceStateManager {
  ...
  public async initializeWorkspaceId() {
    const existingKey = this.context_.workspaceState.get<string>('xbook_WORKSPACE_ID');
    if (!existingKey) {
      const key = `${Date.now()}-${randomInt(0, 100000)}`;
      await this.context_.workspaceState.update('xbook_WORKSPACE_ID', key);
    }
  }

  public getWorkspaceInstance(): string {
    return this.context_.workspaceState.get<string>('xbook_WORKSPACE_ID')!;
  }
