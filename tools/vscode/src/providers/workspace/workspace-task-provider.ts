...
export function activateWorkspaceTaskProvider(xbookManager: xbookManager, context: ExtensionContext) {

  // The task manager
  const taskManager = new WorkspaceTaskManager();

  // If the interpreter changes, refresh the tasks
  context.subscriptions.push(xbookManager.onxbookChanged(async (e: xbookChangedEvent) => {
    if (e.available) {
...
export class WorkspaceTaskManager {
  ...
  public async refresh() {
    ...
    try {
      const tasks = await xbookTaskData(workspace);
      this.setTasks(tasks);
    } catch (err: unknown) {
      console.log("Unable to read xbook task data.");
      console.error(err);
    }
...
async function xbookTaskData(folder: AbsolutePath) {
