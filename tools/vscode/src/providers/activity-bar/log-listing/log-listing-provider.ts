...
export async function activateLogListing(
  ...
  viewServer: xbookViewServer,
  logsWatcher: xbookLogsWatcher
): Promise<[Command[], vscode.Disposable[]]> {

  const kLogListingDir = "xbook.logListingDir";
  const disposables: vscode.Disposable[] = [];

  await vscode.commands.executeCommand(
    "setContext",
    "xbook.haveEvalLogFormat",
    hasMinimumxbookVersion(kxbookEvalLogFormatVersion)
  );
  ...
  disposables.push(vscode.commands.registerCommand('xbook.logListing', async () => {
  ...
  disposables.push(vscode.commands.registerCommand('xbook.logListingReveal', async (uri?: Uri) => {
  ...
  disposables.push(vscode.commands.registerCommand('xbook.logListingRefresh', () => {
    treeDataProvider.refresh();
  }));

  // Register Reveal in Explorer command
  disposables.push(vscode.commands.registerCommand('xbook.logListingRevealInExplorer', async (node: LogNode) => {
  ...
  disposables.push(vscode.commands.registerCommand('xbook.logListingOpenInJSONEditor', async (node: LogNode) => {
    const logUri = treeDataProvider.getLogListing()?.uriForNode(node);
    if (logUri) {
  ...
  disposables.push(vscode.commands.registerCommand('xbook.logListingDeleteLogFile', async (node: LogNode) => {
    const logUri = treeDataProvider.getLogListing()?.uriForNode(node);
    if (logUri) {
  ...
  disposables.push(vscode.commands.registerCommand('xbook.logListingCopyLogPath', async (node: LogNode) => {
    const logUri = treeDataProvider.getLogListing()?.uriForNode(node);
    if (logUri) {
  ...
  disposables.push(vscode.commands.registerCommand('xbook.logListingCopyEditorPath', async (node: LogNode) => {
    const logUri = treeDataProvider.getLogListing()?.uriForNode(node);
    if (logUri) {
  ...
  disposables.push(logsWatcher.onxbookLogCreated((e) => {
...
export async function revealLogListing() {
  ...
  await vscode.commands.executeCommand(`workbench.view.extension.xbook-activity-bar`);
}
