...
export async function activateTaskOutline(
  ...
  const checkxbook = async () => {
    const xbookAvailable = xbookVersion() !== null;
    await commands.executeCommand(
      "setContext",
      "xbook.task-outline-view.noxbook",
      !xbookAvailable
    );
    if (xbookAvailable) {
  ...
  context.subscriptions.push(
    xbookManager.onxbookChanged(async () => {
      await checkxbook();
    })
  );
  await checkxbook();
  ...
  return [
    [
      ...
      new RunSelectedEvalCommand(xbookEvalMgr),
      new DebugSelectedEvalCommand(xbookEvalMgr),
      new EditSelectedTaskCommand(tree, xbookLogviewManager, activeTaskManager),
...
export class TaskOutLineTreeDataProvider
  ...
  public static readonly viewType = "xbook.task-outline-view";
  constructor(
      ...
      this.workspaceMgr.onTasksChanged(
        throttle(
          async (e: TasksChangedEvent) => {
            ...
            await commands.executeCommand(
              "setContext",
              "xbook.task-outline-view.tasksLoaded",
              true
            );
            await commands.executeCommand(
              "setContext",
              "xbook.task-outline-view.noTasks",
              e.tasks?.length === 0
            );
          },
          500,
  ...
  private getNodes(tree: TaskPath[], parent?: TaskTreeItem): TaskTreeItem[] {
    const mode =
      workspace.getConfiguration("xbook").get("taskListView") || "tree";
