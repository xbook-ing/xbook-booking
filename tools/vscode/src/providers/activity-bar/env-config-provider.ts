...
import { kxbookEnvValues } from "../xbook/xbook-constants";
import { xbookVersion } from "../../xbook";
import { xbookChangedEvent, xbookManager } from "../xbook/xbook-manager";
...
const kxbookModels: Record<string, string> = {
  "openai": "0.3.8",
  "anthropic": "0.3.8",
  "google": "0.3.8",
  "mistral": "0.3.8",
...
const xbookModels = () => {
  ...
  return Object.keys(kxbookModels).filter((key) => {
    const ver = kxbookModels[key];
    if (descriptor !== null) {
...
export class EnvConfigurationProvider implements WebviewViewProvider {
  public static readonly viewType = "xbook.env-configuration-view";
  ...
  constructor(
    ...
    private readonly xbookManager_: xbookManager
  ) { }
  ...
  public resolveWebviewView(webviewView: WebviewView) {
    ...
    this.disposables_.push(this.xbookManager_.onxbookChanged(async (e: xbookChangedEvent) => {
      if (e.available) {
        await initMsg();
      } else {
        await noxbookMsg();
  ...
  private htmlForWebview(webview: Webview) {
    ...
    const modelOptions = xbookModels().map((model) => {
...
const envToConfig = (envManager: WorkspaceEnvManager) => {
  ...
  const providerModelStr = env[kxbookEnvValues.providerModel];
  if (providerModelStr) {
  ...
  const logLevel = env[kxbookEnvValues.logLevel];
  if (logLevel) {
    config.logLevel = logLevel;
  }

  const logDir = env[kxbookEnvValues.logDir];
  if (logDir) {
    config.logDir = logDir;
  }

  const maxConnections = env[kxbookEnvValues.connections];
  if (maxConnections) {
    config.maxConnections = maxConnections;
  }

  const maxRetries = env[kxbookEnvValues.retries];
  if (maxRetries) {
    config.maxRetries = maxRetries;
  }

  const timeout = env[kxbookEnvValues.timeout];
  if (timeout) {
    config.timeout = timeout;
  }

  const modelBaseUrl = env[kxbookEnvValues.modelBaseUrl];
  if (modelBaseUrl) {
...
const configToEnv = (config: EnvConfiguration): Record<string, string> => {
  ...
  if (config.provider && config.model) {
    env[kxbookEnvValues.providerModel] = `${config.provider}/${config.model}`;
  } else {
    env[kxbookEnvValues.providerModel] = "";
  }

  env[kxbookEnvValues.logLevel] = config.logLevel || "";
  env[kxbookEnvValues.logDir] = config.logDir || "";
  env[kxbookEnvValues.connections] = config.maxConnections || "";
  env[kxbookEnvValues.retries] = config.maxRetries || "";
  env[kxbookEnvValues.timeout] = config.timeout || "";
  env[kxbookEnvValues.modelBaseUrl] = config.modelBaseUrl || "";
