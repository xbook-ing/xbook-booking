...
export class ShowTaskList implements Command {
  ...
  private static readonly id = "xbook.taskOutlineList";
  public readonly id = ShowTaskList.id;
}

export class RunSelectedEvalCommand implements Command {
  constructor(private readonly xbookEvalMgr_: xbookEvalManager) { }
  async execute(treeItem: TaskTreeItem): Promise<void> {
    ...
    const evalPromise = this.xbookEvalMgr_.startEval(
      toAbsolutePath(path),
      task,
      false
    );
    const resumeFocusCommand = "xbook.task-outline-view.focus";
    scheduleReturnFocus(resumeFocusCommand);
    await evalPromise;
  }
  private static readonly id = "xbook.runSelectedTask";
...
export class DebugSelectedEvalCommand implements Command {
  ...
  async execute(treeItem: TaskTreeItem): Promise<void> {
    ...
    const task =
      treeItem.taskPath.type === "task" ? treeItem.taskPath.name : undefined;
    await this.xbookEvalMgr_.startEval(toAbsolutePath(path), task, true);
  }
  private static readonly id = "xbook.debugSelectedTask";
...
export class EditSelectedTaskCommand implements Command {
  constructor(
    ...
    private xbookLogviewManager_: xbookViewManager,
  ...
  async execute() {
    ...
    if (this.tree_.selection.length === 1) {
      ...
      const logViewColumn = this.xbookLogviewManager_.viewColumn();
  ...
  public static readonly id = "xbook.editSelectedTask";
...
export class CreateTaskCommand implements Command {
  ...
  private static readonly id = "xbook.createTask";
