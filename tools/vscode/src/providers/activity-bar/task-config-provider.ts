...
export type NoxbookCmd = {
  command: "noxbook";
};

export class TaskConfigurationProvider implements WebviewViewProvider {
  public static readonly viewType = "xbook.task-configuration";
  ...
  public async resolveWebviewView(webviewView: WebviewView) {
    ...
    this.disposables_.push(this.xbookManager_.onxbookChanged(async () => {
...
const updateSidebarState = async (taskInfo?: DocumentTaskInfo) => {
  await commands.executeCommand(
    "setContext",
    "xbook.task-configuration.task-active",
    taskInfo !== undefined
  );
};
