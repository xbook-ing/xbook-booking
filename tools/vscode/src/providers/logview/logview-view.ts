...
import { xbookViewPath } from "../../xbook/props";
import {
  xbookChangedEvent,
  xbookManager,
} from "../xbook/xbook-manager";
...
const kLogViewId = "xbook.logview";


export class xbookViewManager {
  constructor(
    private readonly context_: ExtensionContext,
    private readonly webViewManager_: xbookViewWebviewManager,
    private readonly envMgr_: WorkspaceEnvManager,
    logsWatcher: xbookLogsWatcher
  ) {

    this.context_.subscriptions.push(logsWatcher.onxbookLogCreated(async (e) => {
...
export class xbookViewWebviewManager extends xbookWebviewManager<
  xbookViewWebview,
  LogviewState
> {
  constructor(
    ...
    context.subscriptions.push(
      xbookManager.onxbookChanged((e: xbookChangedEvent) => {
  ...
  protected override getWorkspaceState(): LogviewState | undefined {
    const data: Record<string, string> = this.context_.workspaceState.get(this.kxbookViewState, {});
    if (data) {
      return {
  ...
  protected setWorkspaceState(state: LogviewState) {
    void this.context_.workspaceState.update(this.kxbookViewState, {
  ...
  private kxbookViewState = 'xbookViewState';
...
class xbookViewWebview extends xbookWebview<LogviewState> {
  ...
  public setManager(manager: xbookViewWebviewManager) {
    if (this._manager !== manager) {
      this._manager = manager;
    }
  }
  _manager: xbookViewWebviewManager | undefined;
