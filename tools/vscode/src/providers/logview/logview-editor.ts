...
export const kxbookLogViewType = 'xbook-ai.log-editor';


class xbookLogReadonlyEditor implements vscode.CustomReadonlyEditorProvider {

  static register(
    context: vscode.ExtensionContext,
    server: xbookViewServer
  ): vscode.Disposable {
    const provider = new xbookLogReadonlyEditor(context, server);
  ...
  constructor(
    ...
    private readonly server_: xbookViewServer
  ) { }
  ...
  async resolveCustomEditor(
    ...
    let useLogViewer = hasMinimumxbookVersion(kxbookEvalLogFormatVersion);
    if (useLogViewer) {
      const docUri = document.uri.toString();
      if (docUri.endsWith(".json")) {
        const fileSize = await this.server_.evalLogSize(docUri);
        if (fileSize > (1024 * 1000 * 100)) {
          log.info(`JSON log file ${document.uri.path} is to large for xbook View, opening in text editor.`);
          useLogViewer = false;
        }
      }
    }
...
export function activateLogviewEditor(
  context: vscode.ExtensionContext,
  server: xbookViewServer) {
  context.subscriptions.push(xbookLogReadonlyEditor.register(context, server));
}