...
import { xbookEvalCommands } from "./xbook-eval-commands";
...
export async function activateEvalManager(
...
): Promise<[Command[], xbookEvalManager]> {
  // Activate the manager
  const xbookEvalMgr = new xbookEvalManager(stateManager);
  ...
  env.delete("xbook_WORKSPACE_ID");
  env.append("xbook_WORKSPACE_ID", workspaceId);

  return [xbookEvalCommands(xbookEvalMgr), xbookEvalMgr];
}

export class xbookEvalManager {
  ...
  public async startEval(file: AbsolutePath, task?: string, debug = false) {
    // if we don't have xbook bail and let the user know
    if (!xbookVersion()) {
      await window.showWarningMessage(
        `Unable to ${debug ? "Debug" : "Run"
        } Eval (xbook Package Not Installed)`,
        {
          modal: true,
          detail: "pip install --upgrade xbook-ai",
        }
      );
      return;
    }
    ...
    if (
      debug === true &&
      workspace.getConfiguration("xbook").get("debugSingleSample")
    ) {
      args.push(...["--limit", "1"]);
    } else if (limit) {
    ...
    const useSubdirectoryEnvironments = workspace.getConfiguration("xbook").get("useSubdirectoryEnvironments");
    ...
    if (debug) {
      ...
      const env = {
        xbook_WORKSPACE_ID: this.stateManager_.getWorkspaceInstance(),
      };

      await runDebugger(
        xbookBinPath()?.path || "xbook",
        args,
        workspaceDir.path,
        debugPort,
        env,
