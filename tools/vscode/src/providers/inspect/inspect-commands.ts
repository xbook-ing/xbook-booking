...
import { kPythonPackageName } from "../../xbook/props";
import { removeFilesSync } from "../../core/file";


export function activatexbookCommands(
  stateManager: WorkspaceStateManager,
  context: ExtensionContext) {
  const xbookCommands = new xbookCommandDispatcher(stateManager);
  context.subscriptions.push(xbookCommands);
}


export class xbookCommandDispatcher implements Disposable {

  constructor(stateManager: WorkspaceStateManager) {
    // init commands dir and remove any existing commands
    this.commandsDir_ = xbookCommandsDir(stateManager);
...
function xbookCommandsDir(stateManager: WorkspaceStateManager): string {
