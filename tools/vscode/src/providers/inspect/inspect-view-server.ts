...
import { hasMinimumxbookVersion, withMinimumxbookVersion } from "../../xbook/version";
import { kxbookEvalLogFormatVersion, kxbookOpenxbookViewVersion } from "./xbook-constants";
import { xbookEvalLog, xbookEvalLogHeaders, xbookEvalLogs } from "../../xbook/logs";
...
import { xbookManager } from "./xbook-manager";
import { activeWorkspaceFolder } from "../../core/workspace";


export class xbookViewServer implements Disposable {
  constructor(context: ExtensionContext, xbookManager: xbookManager) {
    // create output channel for debugging
    this.outputChannel_ = window.createOutputChannel("xbook View");

    // shutdown server when xbook version changes (then we'll launch
    // a new instance w/ the correct version)
    context.subscriptions.push(
      xbookManager.onxbookChanged(() => {
        this.shutdown();
      })
    );
  }
  ...
  public async evalLogs(log_dir: Uri): Promise<string | undefined> {
    if (this.havexbookEvalLogFormat()) {
  ...
  private async ensureRunning(): Promise<void> {
    ...
    // only do this if we have a new enough version of xbook
    if (!this.havexbookEvalLogFormat()) {
      return;
    }
    ...
    await this.serverStartupLock_.acquire("server-startup", async () => {
      if (this.serverProcess_ === undefined || this.serverProcess_.exitCode !== null) {
        ...
        return new Promise((resolve, reject) => {
          ...
          // find xbook
          const xbook = xbookBinPath();
          if (!xbook) {
            throw new Error("xbook view: xbook installation not found");
          }
          ...
          const options: SpawnOptions = {
            ...
            env: {
              "COLUMNS": "150",
              "xbook_VIEW_AUTHORIZATION_TOKEN": this.serverAuthToken_,
            },
            windowsHide: true
          };
          ...
          this.serverProcess_ = spawnProcess(quote(xbook.path), args.map(quote), options, {
            stdout: onOutput,
            stderr: onOutput,
          }, {
            onClose: (code: number) => {
              this.outputChannel_.appendLine(`xbook View exited with code ${code} (pid=${this.serverProcess_?.pid})`);
            },
            onError: (error: Error) => {
              this.outputChannel_.appendLine(`Error starting xbook View ${error.message}`);
              reject(error);
            },
          });
          this.outputChannel_.appendLine(`Starting xbook View on port ${this.serverPort_} (pid=${this.serverProcess_?.pid})`);
        });
  ...
  private havexbookEvalLogFormat() {
    return hasMinimumxbookVersion(kxbookEvalLogFormatVersion);
  }
...
function evalLogs(log_dir: Uri): Promise<string | undefined> {
  ...
  const response = withMinimumxbookVersion<string | undefined>(
    kxbookOpenxbookViewVersion,
    () => {
      const workspaceRoot = activeWorkspaceFolder().uri;
      const logs = xbookEvalLogs(activeWorkspacePath(), log_dir);
    ...
    () => {
      ...
      return xbookEvalLogs(activeWorkspacePath());
    }
  );
...
function evalLog(
  ...
  return Promise.resolve(
    xbookEvalLog(activeWorkspacePath(), file, headerOnly)
  );
}

function evalLogHeaders(files: string[]) {
  return Promise.resolve(xbookEvalLogHeaders(activeWorkspacePath(), files));
}
