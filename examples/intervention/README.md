...
This is a prototype of an xbook agent with human intervention. It utilises xbook's. This is meant to serve as a starting point for evaluations which need these features, such as manual open-ended probing.

This gives an overview of this task and how to customise it. Note that this task is intended to be run within xbook's approval mode (so you can confirm tool calls) and trace mode (so you can see the conversation with the model). For example:

``` bash
xbook eval intervention.py --approval human --trace
```
...
At the top of `intervention.py`, we have our xbook task:
...
As you can see, this is just a regular xbook evaluation. It should work with any model, and similar features can be integrated into any xbook evaluation that would benefit from human intervention.
