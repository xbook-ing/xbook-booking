...
from xbook.solver import Generate, Solver, TaskState
from xbook.tool import ToolCall, ToolChoice, ToolInfo
...
def langchain_solver(agent: LangChainAgent) -> Solver:
    async def solve(state: TaskState, generate: Generate) -> TaskState:
        # create the xbook model api bridge
        llm = xbookChatModel()
...
class xbookChatModel(BaseChatModel):
    ...
    async def _agenerate(
        ...
        **kwargs: dict[str, Any],
        ...
        # generate
        input = [as_xbookmessage(message) for message in messages]
...
def as_xbookmessage(message: BaseMessage) -> ChatMessage:
    if isinstance(message, SystemMessage):
        return ChatMessageSystem(content=as_xbookcontent(message.content))
    elif isinstance(message, HumanMessage):
        return ChatMessageUser(content=as_xbookcontent(message.content))
    elif isinstance(message, AIMessage):
        return ChatMessageAssistant(
            content=as_xbookcontent(message.content),
            tool_calls=(
                [
                    ToolCall(
                        type="function",
